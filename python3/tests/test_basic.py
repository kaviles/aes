import unittest
from aes.constants import *
from aes.aes import *

class TestAES(unittest.TestCase):

    def test_mod_multiply(self):

        self.assertTrue(mod_multiply(0x5, 0x10) == 0x50)
        self.assertTrue(mod_multiply(0x10, 0x5) == 0x50)
        self.assertTrue(mod_multiply(0x80, 0x80) == 0x4000)
        self.assertTrue(mod_multiply(0xa5, 0xa5) == 0x4411)
        self.assertTrue(mod_multiply(0x3c, 0xa5) == 0x194c)

    def test_bit_count(self):

        self.assertTrue(bit_count(0) == 0)

        value = 1
        for i in range(1, 8):
            
            self.assertTrue(bit_count(value) == i)
            value = value << 1

    def test_galoi_mult(self):

        for i in range(1, 16):
            self.assertTrue(galoi_mult(2**(i-1)) == rcons[i])

    def test_expand_key_1(self):

        # Test cases take from http://www.samiam.org/key-schedule.html

        keySize = 16
        rounds = 11
        exKeySize = rounds * keySize

        key = [0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f]

        exKeyAns = [
            ]

        exKey = expand_key(key, keySize, exKeySize)

        self.assertListEqual(exKey, exKeyAns)

    def test_expand_key_2(self):

        # Test cases take from http://www.samiam.org/key-schedule.html

        keySize = 16
        rounds = 11
        exKeySize = rounds * keySize

        key = [0x69, 0x20, 0xe2, 0x99, 0xa5, 0x20, 0x2a, 0x6d, 0x65, 0x6e, 0x63, 0x68, 0x69, 0x74, 0x6f, 0x2a]
        
        exKeyAns = [
            ]

        exKey = expand_key(key, keySize, exKeySize)

        self.assertListEqual(exKey, exKeyAns)

    def test_add_round_key_0(self):
        exKeySize = 176;

        block = [0x8d, 0x96, 0x4e, 0x53, 0xb9, 0x1f, 0x73, 0x68, 0x99, 0x14, 0xca, 0xdd, 0xea, 0x6b, 0x5e, 0xe9]
        exKey = [
            ]

        ansBlock = [0xe4, 0xb6, 0xac, 0xca, 0x1c, 0x3f, 0x59, 0x05, 0xfc, 0x7a, 0xa9, 0xb5, 0x83, 0x1f, 0x31, 0xc3]

        add_round_key(block, exKey, exKeySize, 0)

        self.assertListEqual(block, ansBlock)

    def test_add_round_key_5(self):
        exKeySize = 176;

        block = [0x8d, 0x96, 0x4e, 0x53, 0xb9, 0x1f, 0x73, 0x68, 0x99, 0x14, 0xca, 0xdd, 0xea, 0x6b, 0x5e, 0xe9]
        exKey = [
            ]

        ansBlock = [0x05, 0x8d, 0x04, 0xe1, 0x03, 0x39, 0x2e, 0xe3, 0x33, 0xc4, 0xe1, 0x1e, 0x8b, 0x2f, 0xa3, 0xb9]

        add_round_key(block, exKey, exKeySize, 5)

        self.assertListEqual(block, ansBlock)

    def test_add_round_key_10(self):
        exKeySize = 176;

        block = [0x8d, 0x96, 0x4e, 0x53, 0xb9, 0x1f, 0x73, 0x68, 0x99, 0x14, 0xca, 0xdd, 0xea, 0x6b, 0x5e, 0xe9]
        exKey = [
            ]

        ansBlock = [0x23, 0x84, 0x32, 0x89, 0x62, 0x58, 0xe8, 0xc0, 0x6b, 0x34, 0x15, 0xe0, 0xa2, 0x33, 0xa8, 0x58]

        add_round_key(block, exKey, exKeySize, 10)

        self.assertListEqual(block, ansBlock)

    # Assume this works correctly. It is entirely dependent on the hard coded SBox.
    # def test_substitute(self):
    #     pass

    def test_shift_rows_left(self):

        block = [0x8d, 0x96, 0x4e, 0x53, 0xb9, 0x1f, 0x73, 0x68, 0x99, 0x14, 0xca, 0xdd, 0xea, 0x6b, 0x5e, 0xe9]
        ansBlock = [0x8d, 0x1f, 0xca, 0xe9, 0xb9, 0x14, 0x5e, 0x53, 0x99, 0x6b, 0x4e, 0x68, 0xea, 0x96, 0x73, 0xdd]

        shift_rows_left(block)

        self.assertListEqual(block, ansBlock)

    def test_mix_columns(self):

        block = [0x8d, 0x96, 0x4e, 0x53, 0xb9, 0x1f, 0x73, 0x68, 0x99, 0x14, 0xca, 0xdd, 0xea, 0x6b, 0x5e, 0xe9]
        ansBlock = [0xbd, 0x3b, 0x72, 0xf2, 0x53, 0x7a, 0xf8, 0x6c, 0x02, 0x29, 0x7e, 0xcf, 0xc5, 0x37, 0x1d, 0xd9]

        mix_columns(block)

        self.assertListEqual(block, ansBlock)

    def test_aes_encrypt_block_1(self):

        rounds = 11

        keySize = 16
        key = [0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c]
    
        exKeySize = keySize * rounds

        block = [0x32, 0x43, 0xf6, 0xa8, 0x88, 0x5a, 0x30, 0x8d, 0x31, 0x31, 0x98, 0xa2, 0xe0, 0x37, 0x07, 0x34]
        ansBlock = [0x39, 0x25, 0x84, 0x1d, 0x02, 0xdc, 0x09, 0xfb, 0xdc, 0x11, 0x85, 0x97, 0x19, 0x6a, 0x0b, 0x32]

        exKey = expand_key(key, keySize, exKeySize)

        aes_encrypt_block(block, exKey, exKeySize, rounds)

        self.assertListEqual(block, ansBlock)

if __name__ == '__main__':
    unittest.main()